...
public class Queries {
	...
	public static void queryListAllGames(String url, String username, String password) {
		try {
			...
			System.out.println(
					"[Game Title]                             [Genre]         [Rating]   [Score] [Platform] [Retailers]");
			...
			while (result.next()) {
				...
				System.out.printf("%-40s %-15s %-10s %-5d   %-10s %-10s \n", gameTitle, gameGenre, gameRating,
						gameScore, gamePlatform, gameRetailers);
			}
			System.out.println();
	...
	public static void queryListSpecificGameInfo(String url, String username, String password, String gameTitleSearch) {
		try {
			...
			System.out.println();
			String sql = "SELECT \n" + "TITLE,\n" + "GENRE,\n" + "RATING,\n" + "SCORE,\n" + "PLATFORM,\n"
					+ "RETAILERS,\n" + "PLOT\n" + " FROM game\n" + " where APPROVED_FLAG = 1 and TITLE like '%"
					+ gameTitleSearch + "%'";
			Statement statement = connection.createStatement();
			ResultSet result = statement.executeQuery(sql);
	...
	public static void queryListSpecificGameDataWithAdditionalInfo(String url, String username, String password,
			String gameTitleSearch) {
		try {
			Connection connection = DriverManager.getConnection(url, username, password);
			System.out.println("Query-> List Game, Publisher, Developer and Composer for Specific Game title like: "
					+ gameTitleSearch);
			System.out.println();
			String sql = "SELECT \n" + "game.TITLE,\n" + "game.GENRE,\n" + "game.RATING,\n" + "game.SCORE,\n"
	...
	public static void queryGamesForSpecificCharacter(String url, String username, String password,
			String gameCharacterSearch) {
		try {
			Connection connection = DriverManager.getConnection(url, username, password);
	...
	public static void queryListSpecificGameSales(String url, String username, String password,
			String gameTitleSearch) {
		try {
			Connection connection = DriverManager.getConnection(url, username, password);
			System.out.println("Query-> List Game, Publisher, Developer and Composer for Specific Game title like: "
			...
			while (result.next()) {
				...
				System.out.printf("%-40s %-4d-%-2d  $%9s  $%9s\n", gameTitle, gameYear, gameMonth, gameSaleAmtFormatted,
						gameSaleRunningAmtFormatted);
